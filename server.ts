...
// src/pipeline.ts
var exports_pipeline = {};
__export(exports_pipeline, {
...
class BrowserPixelLoader {
  ...
  loadFromVideo(video) {
    if (video.readyState < 2) {
      throw new Error('Video is not ready. Wait for the "loadeddata" or "canplay" event before calling getColor/getPalette.');
    }
    const width = video.videoWidth;
    const height = video.videoHeight;
    if (!width || !height) {
      throw new Error("Video has no dimensions. It may not have loaded successfully.");
    }
    const canvas = document.createElement("canvas");
    const ctx = canvas.getContext("2d");
    canvas.width = width;
...
var require_color = __commonJS((exports, module) => {
  ...
  Color.prototype = {
    ...
    level(color2) {
      const contrastRatio2 = this.contrast(color2);
      if (contrastRatio2 >= 7) {
        return "AAA";
      }
      return contrastRatio2 >= 4.5 ? "AA" : "";
    },
    isDark() {
      const rgb = this.rgb().color;
...
async function defaultSharpDecoder(input) {
  let sharpFn;
  try {
    const mod = await Promise.resolve().then(() => __toESM(require_lib(), 1));
    sharpFn = mod.default ?? mod;
  } catch {
    throw new Error("sharp is required for Node.js image loading. Install it with: npm install sharp");
  }
...
var worker = null, blobUrl = null, nextId = 0, pending;
...
var PASSES = [
  ...
  { divisor: 1, progress: 1 }
];
function yieldToMain() {
  return new Promise((resolve) => setTimeout(resolve, 0));
}
async function* extractProgressive(data, width, height, opts, quantizer, signal) {
...
function score(color, target, maxPopulation) {
  ...
  if (l < target.minL || l > target.maxL)
    return -Infinity;
  if (c < target.minC)
    return -Infinity;
...
async function resolveDefaultLoader() {
  ...
  if (isBrowser) {
    const { BrowserPixelLoader: BrowserPixelLoader2 } = await Promise.resolve().then(() => exports_browser);
    return new BrowserPixelLoader2;
  } else {
    const { NodePixelLoader: NodePixelLoader2 } = await Promise.resolve().then(() => exports_node);
    return new NodePixelLoader2;
  }
}
...
function configure(opts) {
  if (opts.loader)
    globalLoader = opts.loader;
  if (opts.quantizer)
    globalQuantizer = opts.quantizer;
}
...
async function getPalette(source, options) {
  ...
  checkAborted(options?.signal);
  if (options?.worker) {
    const { isWorkerSupported: isWorkerSupported2, extractInWorker: extractInWorker2 } = await Promise.resolve().then(() => (init_manager(), exports_manager));
    if (isWorkerSupported2()) {
      const { data, width, height } = await loadPixels(source, options);
      const { createPixelArray: createPixelArray2 } = await Promise.resolve().then(() => (init_pipeline(), exports_pipeline));
...
async function* getPaletteProgressive(source, options) {
  const opts = validateOptions(options ?? {});
  const [pixels, quantizer] = await Promise.all([
    loadPixels(source, options),
    getQuantizer(options?.quantizer)
  ]);
  yield* extractProgressive(pixels.data, pixels.width, pixels.height, opts, quantizer, options?.signal);
}
export {
  getSwatches,
